//
//  StandCardsARView.swift
//  TarotGo
//
//  AR view displaying all 78 tarot cards standing upright on a table
//
...
struct StandCardsARView: UIViewRepresentable {
    ...
    class Coordinator {
        func createStandingCards(in arView: ARView, cardBackStyle: CardBackStyle) {
            ...
            // Layout: Grid arrangement with standing cards
            let allCards = TarotDeck.allCards
            let columns = 13 // 13 columns
            let rows = 6 // 6 rows
            
            let cardSpacing: Float = 0.01 // 1cm spacing between cards
            let totalWidth = Float(columns) * (cardWidth + cardSpacing)
            let totalDepth = Float(rows) * (cardWidth + cardSpacing) // Using width for depth since cards are standing
            ...
            for (index, card) in allCards.enumerated() {
                ...
                let z = startZ + Float(row) * (cardWidth + cardSpacing) + cardWidth / 2
                ...
                if let cardEntity = createCardEntity(
                    ...
                    // Position on horizontal plane, standing upright
                    // Cards stand on their bottom edge
                    cardEntity.position = SIMD3(x: x, y: cardHeight / 2, z: z)
                    
                    // No rotation - cards stand upright by default
                    // (already vertical in their default orientation)
